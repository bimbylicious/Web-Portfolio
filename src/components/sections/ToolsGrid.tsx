...
// backgrounds; each brand's own guidelines specify white for dark backgrounds instead.
const DARK_BG_OVERRIDE = new Set(['Next.js 16', 'shadcn/ui', 'Vercel']);

const SIMPLE_ICON_LOGOS: Partial<
  Record<(typeof TOOLS)[number]['name'], { path: string; hex: string }>
> = {
...
function AwsLogo() {
  return (
    <svg viewBox="0 0 128 128" className="h-6 w-6" aria-hidden="true">
      <path
        fill="#F7A80D"
...
function ToolLogo({ name }: { name: string }) {
  if (name === 'AWS') return <AwsLogo />;
  if (name === 'Playwright') return <PlaywrightLogo />;

  const logo = SIMPLE_ICON_LOGOS[name as (typeof TOOLS)[number]['name']];
  if (!logo) return null;

  const color = DARK_BG_OVERRIDE.has(name) ? '#FFFFFF' : `#${logo.hex}`;
